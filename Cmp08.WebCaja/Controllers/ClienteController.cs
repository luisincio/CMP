...
namespace Cmp08.WebCaja.Controllers
{
    ...
    public class ClienteController : Controller
    {
        ...
        #region Modal de Registro Persona Natural

        public ActionResult Registro(string vId = "", int vTipo = (int)EnumTipoPersonaGP.NATURAL)
        {
            ViewBag.LstDocumento = (from x in OMaestra.List(EnumMaestras.TIPO_DOCUMENTO) where x.Id_Maestras == (int)EnumTipoDocumento.DNI || x.Id_Maestras == (int)EnumTipoDocumento.RUC select x).ToList();
            var Temp = new Cliente_LN().Obtener(vId, vTipo);
            Temp.Clase = AppSession.Usuario.Clase;
            Temp.TipoDocumento = (int)EnumTipoDocumento.DNI;
            return PartialView("_EditarCliente", Temp);
        }

        [HttpPost]
        public ActionResult Registro(ClienteClover vClienteClover)
        {
            vClienteClover.Usu_Ingreso = vClienteClover.Usu_Modifica = AppSession.Usuario.Usuario;
            vClienteClover.TipoPersona = (int)EnumTipoPersonaGP.NATURAL;
            if (ModelState.IsValid) new Cliente_LN().GrabarDatos(vClienteClover);
            ViewBag.LstDocumento = (from x in OMaestra.List(EnumMaestras.TIPO_DOCUMENTO) where x.Id_Maestras == (int)EnumTipoDocumento.DNI || x.Id_Maestras == (int)EnumTipoDocumento.RUC select x).ToList();
            return PartialView("_EditarCliente2", vClienteClover);
        }
        #endregion

        #region Modal de Registro Persona Juridica


        public ActionResult Registro2(string vId = "", int vTipo = (int)EnumTipoPersonaGP.JURIDICA)
        {
            ViewBag.LstDocumento = (from x in OMaestra.List(EnumMaestras.TIPO_DOCUMENTO) where x.Id_Maestras == (int)EnumTipoDocumento.RUC select x).ToList();
            ViewBag.LstSector = OMaestra.List(EnumMaestras.SECTOR);
            var Temp = new Cliente_LN().Obtener(vId, vTipo);
            Temp.Clase = AppSession.Usuario.Clase;
            Temp.TipoDocumento = (int)EnumTipoDocumento.RUC;
            return PartialView("_EditarEmpresa", Temp);
        }

        [HttpPost]
        public ActionResult Registro2(ClienteClover vClienteClover)
        {
            vClienteClover.Usu_Ingreso = vClienteClover.Usu_Modifica = AppSession.Usuario.Usuario;
            vClienteClover.ApellidoMaterno = vClienteClover.ApellidoPaterno = vClienteClover.SegundoNombre = vClienteClover.Celular = vClienteClover.Email = "";
            vClienteClover.TipoPersona = (int)EnumTipoPersonaGP.JURIDICA;
            vClienteClover.Id_Consejo = AppSession.Usuario.Id_Consejo_Regional;
            if (ModelState.IsValid)
                new Cliente_LN().GrabarDatos(vClienteClover);
            else if (
                (ModelState.Where(X => X.Value.Errors.Count() > 0).Count() <=4) && ModelState.Where(X => X.Value.Errors.Count() > 0).Where(x => x.Key == "ApellidoPaterno" || x.Key == "ApellidoMaterno" || x.Key == "Celular" || x.Key == "Email").Count() <=4)
                new Cliente_LN().GrabarDatos(vClienteClover);
            ViewBag.LstDocumento = (from x in OMaestra.List(EnumMaestras.TIPO_DOCUMENTO) where x.Id_Maestras == (int)EnumTipoDocumento.RUC select x).ToList();
            ViewBag.LstSector = OMaestra.List(EnumMaestras.SECTOR);
            return PartialView("_EditarEmpresa2", vClienteClover);
        }
        ...
        public ActionResult BuscarClienteGP(string vNombre = "FULLSITO")
        {
            if (vNombre == "FULLSITO")
                return PartialView("_BuscarCliente", new Orden_Compra_LN().List_Proveedor(vNombre));
            else
                return PartialView("_BuscarClienteDetalle", new Orden_Compra_LN().List_Proveedor(vNombre));
        }

        public JsonResult ListarDireccionGP(string vCliente)
        {
            return Json(new Cuentas_Cobrar_LN().List_Direccion(vCliente), JsonRequestBehavior.AllowGet);
        }
